#Description: This code is am example for generating the AWS Signature V4 and use it to call DF SP API
...
#BEGIN***********For calling GET Transaction Status*****************
...
region = 'us-east-1'
endpoint = 'https://sellingpartnerapi-na.amazon.com/vendor/directFulfillment/transactions/v1/transactions'
#endpoint = 'https://sandbox.sellingpartnerapi-na.amazon.com/vendor/directFulfillment/transactions/v1/transactions'

#LWA Access Token recieved in response of the above API call
access_token = r_access_token

#Transaction ID received in the response of the POST Submit Shipping Label API call
transaction_id = ""
...
#canonical_uri is the URI-encoded version of the absolute path component of the URI—everything starting with the "/" that follows the domain name and up to the end of the string or to the question mark character ('?') if you have query string parameters 
canonical_uri = "/vendor/directFulfillment/transactions/v1/transactions"+ "/" + transaction_id
...
request_url = endpoint + '/' + transaction_id
#request_url = endpoint
